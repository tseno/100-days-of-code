...
- 副作用を実行する
- App コンポーネントに再描画されるたびに、`<p id="effectHook"></p>`にテキストを描画する副作用が実行される。
...
- 以下のように、第２引数を`[]`にすると、１度のみ副作用が実行される
...
- メモ化された値を返すフック
- `useMemo(() => double(count1), [count1]);` で、count1 が変化したら、double(count1)で値を取得、変化しなかったらキャッシュを使う。
